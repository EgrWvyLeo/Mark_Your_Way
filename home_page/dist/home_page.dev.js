...
landmark_like.addEventListener("mouseover", function () {
  heart.style.fill = "#d7443e"; // heart.classList.toggle("heartToggle");
});
landmark_like.addEventListener("mouseout", function () {
  heart.style.fill = "black"; // heart.classList.toggle("heartToggle");
...
pin_1.onclick = function () {
  ...
  next.addEventListener("click", function () {
    ...
    if (count == totalQuestion[landmarkCount] - 1) {
      result.style.display = "block"; // skip.style.display = "none";
  ...
  var minutes = Math.floor((timer - timerCounter) % (1000 * 60 * 60) / (1000 * 60));
  var seconds = Math.floor((timer - timerCounter) % (1000 * 60) / 1000);
  setInterval(function () {
    resultBox_countdown.innerHTML = days + "d " + hours + "h " + minutes + "m " + seconds + "s "; // timer -= 1000;

    timerCounter += 1000;
  }, 1000); // get the user score if he successfully get the correct answer
  ...
  for (var i = 0; i < answer.length; i++) {
    answer[i].addEventListener("click", function () {
      ...
      if (userSelect == correctAnswer) {
        ...
        scoreEachLandmark = scoreEachLandmark + allTest[landmarkCount][count].point;
        questionPass++;
